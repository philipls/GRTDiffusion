...
void grtrot600(double *T, double *Gt, double *Theta, double *Ptheta, double *Mt, 
...
unsigned n, m;
    ...
    if (nrhs != 3) {
         mexErrMsgTxt("grtrot600: Requires 3 input args.");
    } else if (nlhs != 5) {
        mexErrMsgTxt("grtrot600: Requires 5 output args."); 
    }
    ...
    */
    ...
    if (!mxIsDouble(prhs[0]) || !(m * n == NP)) {
        ...
        mexErrMsgTxt("grtrot600: Wrong size P");
    } else {
        P = mxGetPr(prhs[0]);
    }
    /* tmax */
    ...
    if (!mxIsDouble(prhs[1]) || !(m * n == 1)) {
        mexErrMsgTxt("grtrot600: tmax must be a scalar");
    } else { 
    ...
    if (!mxIsDouble(prhs[2]) || !(m * n == 1)) {
        mexErrMsgTxt("grtrot600: badi must be a scalar");
    } else {
    ...
    */
    ...
    grtrot600(T, Gt, Theta, Ptheta, Mt, P, tmax, badix);
}
